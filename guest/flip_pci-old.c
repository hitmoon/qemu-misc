...
static int flip_fifo_init(struct flip_fifo *fifo)
{
	...
	fifo->data = (char *)kmalloc(DEFAULT_FIFO_SIZE, GFP_KERNEL);
	if (fifo->data == NULL)
		return -1;
	fifo->head = 0;
	fifo->rear = 0;
...
static int flip_fifo_get_data(struct flip_fifo *fifo, unsigned char *p)
{
	...
	*p = fifo->data[fifo->head];
	fifo->head = (fifo->head + 1) % DEFAULT_FIFO_SIZE;
	return 0;
}
...
{
	if (flip_fifo_is_full(fifo))
		return -1;
	
	fifo->data[fifo->rear] = ch;
	fifo->rear = (fifo->rear + 1) % DEFAULT_FIFO_SIZE;
...
static irqreturn_t flip_handler(int irq, void *dev_id)
{
	...
	for (i = 0; i < FLIP_REG_LEN; i++) {
		...
		if (ret < 0) {
			printk(KERN_ERR "flip fifo full !\n");
			goto fail;
		}
	}
...
static ssize_t flip_char_read(struct file *flip, char *buff, size_t count, loff_t *f_pos)
{
	...
	if (!data)
		return -ENOMEM;
	i = 0;
	while (!flip_fifo_is_empty(&dev->fifo_out) && i < count) {
		flip_fifo_get_data(&dev->fifo_out, &data[i++]);
	}
	up(&dev->sem);
